...
## 📌 Feedback from you:
RustDU 🦀 is actively maintained as a pet project alongside my university studies 🎓. Updates 🔄 and new features ✨ might occasionally land a little slower during semesters 📚, but I'm fully committed to fixing bugs 🐛 and improving the tool 🛠️.
...
- 📈 **Percentage bars** — each item shows its share of the total size
- 🔄 **Sorting** — by size, name, or modified time (`s` / `n` / `t` keys)
- 🗑️ **Delete files/folders** (with confirmation, batch via Space)
- 🧭 **Navigate** — jump to any path (`g`), go back (`Backspace`), enter folders (`Enter`)
- 🔄 **Refresh** current directory and clear size cache (`r`)
- ❓ **Help screen** (`?`)
...
- `--json` — dump the report as JSON and exit (no TUI, great for scripts)
